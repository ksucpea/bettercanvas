...
/*
function changeAssignmentDueDate() {
    console.log("includes?", current_page.includes("assignments/"));
    if (current_page.includes("assignments/")) {
        try {
            document.addEventListener("DOMContentLoaded", () => {
                let date = document.querySelector(".date_text");
                const dayNames = ["Sunday", "Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday"];
                const now = new Date();
                const dayNum = dayNames.indexOf(date.querySelector(".display_date").textContent);
                console.log(now, new Date(now.getTime() + ((dayNum - now.getDay()) * 86400000)));
                //now.setDate(now.get)
...
function setupGPACalc() {
    ...
    try {
        grades.then(result => {
            ...
            let container2 = makeElement("div", "bettercanvas-gpa-card", document.querySelector(".ic-DashboardCard__box__container"));
