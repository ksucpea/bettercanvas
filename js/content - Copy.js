...
function setAssignmentState(id, updates) {
    ...
    console.log(options.assignment_states, states, Object.keys(states));
    if (Object.keys(states).length > 80) {
        let keys = Object.keys(states).sort(a, b => states[a].expire - states[b].expire);
        keys.splice(20);
        let newStates = {};
        keys.forEach(key => {
    ...
    chrome.storage.sync.set({ assignment_states: states });
}
...
function createTodoHeader(location) {
    ...
    let headerText = makeElement("span", "bettercanvas-todo-header", todoHeader, {"className": "bettercanvas-todo-header", "textContent": "To Do"});
...
function createTodoViewMore(location, type) {
    let viewMoreButton = makeElement("button", "bettercanvas-custom-btn", location, "View More");
...
function loadBetterTodo() {
    console.log("loading better todo", moreAnnouncementCount, moreAssignmentCount);
    if (options.better_todo !== true) return;
    try {
            ...
            chrome.storage.sync.get(options.custom_assignments_overflow, storage => {
                ...
                items.forEach((item, index) => {
                    ...
                    if (item.plannable_type === "announcement") {
                        ...
                        if (filter === "done" && (now <= date)) return;
                    ...
                    if (filter === "todo" && item.planner_override && item.planner_override.marked_complete === true) return;

                    let listItemContainer = document.createElement("div");
                    listItemContainer.classList.add("bettercanvas-todo-container");
                    listItemContainer.innerHTML = '<div class="bettercanvas-hover-preview"><p class="bettercanvas-preview-text"></p></div><div class="bettercanvas-todo-actions"></div><div class="bettercanvas-todo-icon"></div><a class="bettercanvas-todo-item"></a><button class="bettercanvas-todo-actions-btn"><i class="icon-more" aria-hidden="true"></i></button>';
                    ...
                    if (itemState) {
                        if (itemState["lbl"] && itemState["lbl"] !== "None") {
                            makeElement("span", "bettercanvas-todo-label", listItem, itemState["lbl"]);
                        }
                        if (itemState["crs"] && itemState["crs"] === true) {
                            listItemContainer.querySelector(".bettercanvas-todo-item").style.textDecoration = "line-through";
                        } else if (itemState["crs"] && itemState["crs"] === false) {
                            listItemContainer.querySelector(".bettercanvas-todo-item").style.textDecoration = "auto";
                        }
                    }
                    ...
                    let removeBtn = makeElement("div", "bettercanvas-todo-action", actions, "Remove");
                    removeBtn.style.display = "block";
                    /*
                    removeBtn.addEventListener("click", () => {
                        setAssignmentState(item.plannable_id, { "rem": true, "expire": item.plannable_date });
                        loadBetterTodo();
                    });
                    */
                    let crossOffBtn = makeElement("div", "bettercanvas-todo-action", actions, "Cross off");
                    crossOffBtn.style.display = "block";
                    crossOffBtn.addEventListener("click", () => {
                        setAssignmentState(item.plannable_id, { "crs": listItemContainer.querySelector(".bettercanvas-todo-item").style.textDecoration === "line-through" ? false : true, "expire": item.plannable_date });
                        loadBetterTodo();
                    });
                    //const labels = ["None", "Exam", "Quiz", "Assignment", "Announcement", "Ungraded"];
                    makeElement("span", "bettercanvas-todo-action-tag", actions, "Label:")
                    let labelInput = makeElement("input", "bettercanvas-todo-input", actions);
                    labelInput.addEventListener("change", (e) => {
                        setAssignmentState(item.plannable_id, { "lbl": e.target.value, "expire": item.plannable_date });
                        loadBetterTodo();
                    })

                    /*
                    labels.forEach(label => {
                        let btn = makeElement("span", "bettercanvas-todo-label-btn", actions, label);
                        btn.addEventListener("click", () => {
                            setAssignmentState(item.plannable_id, { "lbl": label, "expire": item.plannable_date });
                            loadBetterTodo();
                        });
                    })
                    */
...
*/
                ...
                if (assignmentsToInsert.length > 0) {
                    let i;
                    console.log(maxAssignmentCount, assignmentsToInsert.length);
                ...
                if (announcementsToInsert.length > 0) {
                    console.log(announcementsToInsert);
                    let i;
