...
function setupBetterTodo() {
    ...
    try {
        ...
        let list = document.querySelector(".Sidebar__TodoListContainer");
...
function loadBetterTodo() {
    ...
    try {
        ...
        chrome.storage.local.get("custom_assignments", storage => {
            ...
            if (todoAnnouncements.children.length > 0) {
                for (let i = todoAnnouncements.children.length - 1; i >= itemCount; i--) {
                    todoAnnouncements.removeChild(todoAnnouncements.children[i]);
                }
            } else {
...
function insertCardAssignments(c = null) {
    try {
        assignments.then(data => {
            assignmentData = assignmentData === null ? data : assignmentData;
            chrome.storage.local.get("custom_assignments", storage => {
...
function cleanCustomAssignments() {
    chrome.storage.local.get("custom_assignments", storage => {
        const now = new Date();
        for (let i = 0; i < storage.custom_assignments.length; i++) {
            if (new Date(storage.custom_assignments[i].plannable_date) < now) {
                storage.custom_assignments.splice(i, 1);
            }
        }
...
function condenseCards() {
    if (options.condensed_cards !== true) return;
    let style = document.createElement('style');
