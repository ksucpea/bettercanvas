...
function startExtension() {
    toggleDarkMode();
    const optionsList = ["assignments_due", "dashboard_grades", "gradient_cards", "auto_dark", "auto_dark_start", "auto_dark_end", 'num_assignments', 'assignments_done', "gpa_calc", "assignment_date_format", "assignments_quizzes", "assignments_discussions", "dashboard_notes", "dashboard_notes_text", "improved_todo", "new_install"];
...
function formatDate(date, submissions) {
    ...
    return { dueSoon: dueSoon, date: months[date.getMonth()] + " " + date.getDate() + " at " + (date.getHours() - (date.getHours() > 12 ? 12 : 0)) + ":" + (date.getMinutes() < 10 ? "0" : "") + date.getMinutes() + (date.getHours() >= 12 ? "pm" : "am") + " (" + fromNow + ")" };
}
...
function setupBetterTodo() {
    ...
    try {
        ...
        if (list.childElementCount > 0 && list.children[0].id != "bettercanvas-todo-list") {
            ...
            assignments.then(data => data.forEach(item => {
                ...
                if (item.plannable_type === "announcement" && announcementCount < 5 || (item.plannable_type === "assignment" || item.plannable_type === "quiz" || item.plannable_type === "discussion_topic") && todoCount < 5 && date >= new Date()) {
                    if (!item.planner_override || item.planner_override.marked_complete === false) {
                        ...
                        listItemContainer.querySelector(".bettercanvas-todo-item").href = domain + item.html_url;
                        ...
                        let format = formatDate(date, item.submissions);
...
function getAssignmentData() {
    if ((current_page === "/" || current_page === "") && options.assignments_due === true || options.improved_todo === true) {
        let weekAgo = new Date(new Date() - (7 * 24 * 60 * 60 * 1000));
    ...
    if (current_page === "/grades" && options.gpa_calc === true || (current_page === "/" || current_page === "") && (options.dashboard_grades === true && options.assignments_due === true)) {
...
function insertAssignments(assignments, card_order) {
    ...
    for (let i = 0; i < cards.length; i++) {
        ...
        assignments.forEach(function (assignment) {
            if (card_order[i].id === assignment.course_id && new Date(assignment.plannable_date) > new Date() && count < options.num_assignments) {
                if (assignment.plannable_type === "assignment" || assignment.plannable_type === "quiz" && options.assignments_quizzes || assignment.plannable_type === "discussion_topic" && options.assignments_discussions) {
                    ...
                    let destination = assignment.plannable_type === "quiz" ? "quizzes" : assignment.plannable_type === "discussion_topic" ? "discussion_topics" : "assignments";
                    assignmentName.setAttribute("href", `${domain}/courses/${card_order[i].id}/${destination}/${assignment.plannable_id}`);
                }
            };
        });
        if (count === 0) {
...
function setupCustomURL() {
    ...
    if (domain.includes("canvas") || domain.includes("instructure") || domain.includes("learn") || domain.includes("school")) {
        console.log("Better Canvas - url detected automatically");
        setURL();
    } else {
        console.log("Better Canvas - url couldn't be detected automatically");
        document.addEventListener("DOMContentLoaded", () => {
            let setupContainer = makeElement("div", "bettercanvas-customurl-setup", document.body);
            makeElement("p", "bettercanvas-customurl-message", setupContainer, "Thanks for installing Better Canvas!");
            makeElement("p", "bettercanvas-customurl-message", setupContainer, "To start the extension, press the button below when you are on your Canvas page.").style.marginTop = "12px";
            makeElement("button", "bettercanvas-customurl-confirm", setupContainer, "This is my Canvas page, let's go!").addEventListener("click", () => {
                setURL();
            });
        });
    }
