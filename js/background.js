...
chrome.runtime.onInstalled.addListener(function(){
    let optionslist = ['new_install','assignments_due', 'gpa_calc', 'dark_mode', 'gradient_cards', 'link_preview', 'auto_dark', 'auto_dark_start', 'auto_dark_end', 'dark_css'];
    optionslist.forEach(function(option) {
        chrome.storage.local.get([option], function(result) {
            if(Object.keys(result).length === 0) {
                switch(option) {
                    case 'new_install': chrome.storage.local.set({new_install: true}); break;
                    ...
                    .then(function (result) {
                        chrome.storage.local.set({dark_css: result.darkcss});
                    });
                    break;
                }
            }
